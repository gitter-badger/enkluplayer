...
public class ManualCameraControl : MonoBehaviour
{
    ...
    private Vector3 GetCameraControlTranslation()
    {
        ...
        {
            deltaPosition += InputCurve(Input.GetAxisRaw(MoveHorizontal)) * this.transform.right;
        }
        ...
        {
            deltaPosition += InputCurve(Input.GetAxisRaw(MoveVertical)) * this.transform.forward;
        }
